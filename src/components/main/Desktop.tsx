...
// import { WindowData, DesktopIconData } from './types';
import type { WindowData, DesktopIconData } from './types';
import { Brain, Mail, Globe, Gamepad2, Cloud, Music, Settings } from 'lucide-react';
...
const Desktop = () => {
  ...
  const [currentWallpaper, setCurrentWallpaper] = useState('linear-gradient(135deg, hsl(220 13% 18%), hsl(240 15% 12%), hsl(260 20% 8%))');
  const [isDarkMode, setIsDarkMode] = useState(true);
  const [desktopIcons, setDesktopIcons] = useState<DesktopIconData[]>([
    { id: 'lumina-icon', name: 'Lumina AI', icon: <Brain className="w-8 h-8" />, position: { x: 50, y: 150 }, appId: 'lumina-ai' },
    { id: 'sampark-icon', name: 'Sampark AI', icon: <Mail className="w-8 h-8" />, position: { x: 50, y: 250 }, appId: 'sampark-ai' },
    { id: 'ion-icon', name: 'ION Browser', icon: <Globe className="w-8 h-8" />, position: { x: 150, y: 150 }, appId: 'ion-browser' },
    { id: 'quizaki-icon', name: 'Quizaki', icon: <Gamepad2 className="w-8 h-8" />, position: { x: 150, y: 250 }, appId: 'quizaki' },
    { id: 'weather-icon', name: 'Weather', icon: <Cloud className="w-8 h-8" />, position: { x: 250, y: 150 }, appId: 'weather-app' },
    { id: 'sonic-icon', name: 'Sonic Boom', icon: <Music className="w-8 h-8" />, position: { x: 250, y: 250 }, appId: 'sonic-boom' },
  ]);

  const getAppComponent = (appId: string) => {
    switch (appId) {
      case 'settings': return <SettingsApp currentWallpaper={currentWallpaper} onWallpaperChange={setCurrentWallpaper} isDarkMode={isDarkMode} onDarkModeToggle={() => setIsDarkMode(!isDarkMode)} onResetSettings={() => { setCurrentWallpaper('linear-gradient(135deg, hsl(220 13% 18%), hsl(240 15% 12%), hsl(260 20% 8%))'); setIsDarkMode(true); }} />;
      case 'lumina-ai': return <LuminaAIApp />;
      case 'sampark-ai': return <SamparkAIApp />;
      case 'ion-browser': return <IONBrowserApp />;
      case 'quizaki': return <QuizakiApp />;
      case 'weather-app': return <WeatherApp />;
      case 'sonic-boom': return <SonicBoomApp />;
      default: return <div className="p-6 text-white">App not found</div>;
    }
  };
  ...
  const handleIconDragUpdate = (iconId: string, position: { x: number; y: number }) => {
  ...
  const handleContextMenuAction = (action: string) => {
    switch (action) {
      ...
      case 'settings':
        ...
        break;
      case 'about':
        openWindow('about', 'About Aditya OS', 
